...
describe GroupAbbreviation do
  ...
  it "is invalid if the same value is entered twice on the same group at once" do
    stamm.attributes = {abbreviations_attributes: [{value: "adl"}, {value: "ADL"}]}

    expect(stamm).not_to be_valid
    expect(stamm.errors[:"abbreviations.value"]).to be_present
  end

  it "ignores siblings marked for destruction" do
    existing = stamm.abbreviations.create!(value: "adl")
    stamm.attributes = {abbreviations_attributes: [{id: existing.id, _destroy: "1"}, {value: "adl"}]}
