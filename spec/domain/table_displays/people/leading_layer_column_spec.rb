...
describe TableDisplays::People::LeadingLayerColumn, type: :helper do
  ...
  describe "in view" do
    ...
    it "requires show as permission" do
      expect(display.required_permission(:leading_layer)).to eq :show
    end

    it "renders Hauptgruppierung as header" do
      display.render(:leading_layer)
  ...
  describe "in export" do
    ...
    it "uses the plain group name, not a link" do
      expect(resolve_export_value(:leading_layer).to_s).to eq("Adler")
    end
    ...
    it "is empty when person holds no active role at all" do
      ...
      expect(resolve_export_value(:leading_layer).to_s).to eq("")
    end
