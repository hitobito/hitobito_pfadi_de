...
describe Person do
  ...
  describe "#membership_group" do
    let(:person) { people(:member) }
    let(:group) { groups(:adler_mitglieder) }

    it "is the layer group of the person's active Ordentliche Mitgliedschaft" do
      expect(person.membership_group).to eq group.layer_group
      expect(person.membership_group_role).to eq roles(:paying_member)
    end
    ...
    it "is nil when the person holds no active role at all" do
      ...
      expect(person.membership_group_role).to be_nil
      expect(person.membership_group).to be_nil
    end
    ...
    it "counts Foerdermitgliedschaft as a membership role too" do
      ...
      expect(person.membership_group_role).to eq foerdermitgliedschaft
    end
